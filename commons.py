...
def striking_FF(list_o_entries):
    ...
    for entry in list_o_entries:
        if ('strikes' in list(entry.keys()) and entry.get('strikes')<0) or ('0_th_var' in list(entry.keys())):
            ...
            change = min(np.random.poisson(1)), len(entry.get('ranks')-1) #with this we ensure that the chosen thing is within range of the dictionary
            ...
            entry.update({
                'strikes' : 10 * dd_responsivity['current']
            })
...
def new_targeting_FF(name):
    ...
    eta_acc = AGENTS_r[name][0]['dd_eta']['current'] #this way we get the first accepted ETA, which is the duration of the target
...
def finding_FF(complete_dictionary, what, how='highest', cond_dict=False):
...
def weighting_FF(time, var, weight, dictionary, Type = 'average', EorM=False, demand=False, public=False, discount=0):
    ...
    for period in time_loop:
        if len(dictionary.get(period)) > 0:
            ...
            if (var, weight == 'price', 'MWh'):
                if demand!=True:
                    var_dict={0: STARTING_PRICE, 1: STARTING_PRICE, 2: STARTING_PRICE, 3: STARTING_PRICE, 4:STARTING_PRICE, 5:STARTING_PRICE}
                else:
    ...
    for _ in list(denominator.keys()):
        ...
        j = var_dict.get(_) #EDIT AUGUST 2021, we used to have this if to avoid zero pricing. Not necessary anymore, in fact we will need it. if var_dict.get(_) else STARTING_PRICE this is to avoid zero pricing for sources not used yet
...
def private_reporting_FF(genre, EorM = False):
    ...
    for source in Sources:
        ...
        current = finding_FF(AGENTS.get(env.now-1), specific_profits, 'sum', {'genre' : genre}) if EorM == False else finding_FF(AGENTS.get(env.now-1), specific_profits, 'sum', {'genre' : genre, 'EorM' : EorM}) #we now have the current profits for that source in that agent
    ...
    if rationale == 'green':
        ...
        for _ in AGENTS.get(env.now-2):
            ...
            if agent.get('genre') == 'TP':
                ...
                var = agent.get('RandD') if rationale == 'innovation' else agent.get('capacity') #if the rationale is innovation we are looking into R&D, if not then the rationale is internalization and we are looking at the productive capacity
...
def evaluating_FF(name):
    ...
    if avg_time < env.now-1:
        ...
        if ((present > (1 + responsiveness) * hist) and more_the_better == True) or ((present < (1 - responsiveness) * hist) and more_the_better == False):
        ...
        elif ((present > (1 + responsiveness) * hist) and more_the_better == False) or ((present < (1 - responsiveness) * hist) and more_the_better == True):
            #current is better than hist, so now we run the distribution          
            strikes = [-3,-1] if dist > ratio else [-1,0]
            if more_the_better == True:
                action = 'change' if change[genre] == True else 'keep'
...
def private_deciding_FF(name):
    ...
    if genre == 'BB':
        means += sums/NUMBER_OF_BB
    elif genre == 'EP':
        means += sums/NUMBER_OF_EP
    else:
        means += sums/NUMBER_OF_TP
...
def public_deciding_FF(name):
    ...
    if (2*avg_time > env.now-1 and t_1['dd_rationale']['current'] == t_2['dd_rationale']['current'] == rationale and t_1['dd_target']['current'] == t_2['dd_target']['current'] == target):
...
def npv_generating_FF(interest, time, lumps, MW, building_t, capex, opex, p, capacity_factor, ammort_t, cash_flow_RISK=0, financing_RISK=0, true_capex_and_opex=False):
...
def policymaking_FF(dd, Policies, value, add=False):
    ...
    if isinstance(dd['current'], int):
        ...
        if dd['current'] != ('green', 'innovation', 'capacity'):
            ...
            if 'auction_countdown' in list(Policy.keys()):
                #it was an auction
...
def financing_FF(genre, target, name, my_wallet, my_receivables, value, financing_index, guaranteeing=False):
    ...
    interest_r = r*(1+value) if genre == 'BB' else r*(1-value) #if the agent is a private bank it increases the general interest rate by (1+risk), on the other hand, if the agent is the development bank it reduces the general interest rate by (1-effort)
    ...
    if env.now > 0 and len(CONTRACTS.get(env.now-1)) > 0 and car_ratio >= BASEL:
        for _ in CONTRACTS.get(env.now-1):
            ...
            if i.get('receiver') == name and i.get('status') == 'project':
                ...
                NPV = npv_generating_FF(interest_r, i.get('lifetime'), i.get('Lumps'), i.get('capacity'), i.get('building_time'), i.get('CAPEX'), i.get('OPEX'), price, i.get('CF'), AMMORT, cash_flow_risk, financing_risk, True)
    ...
    for project in adressed_projects_NPVs:
        ...
        receiv_value = new_receivables.get(k) + j.get('CAPEX') * ((interest_r) ** j.get('building_time')) if guaranteeing == False else new_receivables.get(k)
...
def dd_dict_generating_FF(current, source=True, zero_to_start=True):
    """
